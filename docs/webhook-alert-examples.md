...
async function sendToSlack(alertData, env) {
  ...
  const slackMessage = {
    ...
    attachments: [{
      ...
      footer: 'Atlas Firebird Security Monitor',
...
async function sendToEmail(alertData, env) {
  ...
  const emailContent = `
SECURITY ALERT - Atlas Firebird System
=====================================
...
This is an automated alert from the Atlas Firebird security monitoring system.
Please investigate this event and take appropriate action if necessary.

Dashboard: https://intake-speed.bytecrash.workers.dev/admin
Logs: https://dash.cloudflare.com/workers
  `;

  const msg = createMimeMessage();
  msg.setSender({ name: "Atlas Firebird Security", addr: env.FROM_EMAIL });
...
async function testAlertThresholds() {
  ...
  for (let i = 0; i < 15; i++) {
    await fetch('https://intake-speed.bytecrash.workers.dev/admin', {
      headers: {
...
*These webhook examples provide a foundation for automated security monitoring and response for the Atlas Firebird intake system.*